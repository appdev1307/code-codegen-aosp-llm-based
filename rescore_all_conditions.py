...
def score_structure(content: str, agent_type: str) -> float:
    if agent_type == "aidl":
        ...
        # New contract: allow rules against the shared hal_vehicle_vss
        # domain, granting vss_hw_data_file access (real file I/O backing
        # the CPP agent's simulated hardware register interface).
        checks = ["hal_vehicle_vss", "vss_hw_data_file", ";", "allow"]
        hits = sum(1 for c in checks if c in content)
...
def score_coverage(content: str, agent_type: str) -> float:
    ...
    if agent_type == "aidl":
        ...
        terms = ["hal_vehicle_vss", "vss_hw_data_file", "allow", "vendor", "vehicle", ";"]
    elif agent_type == "build":
