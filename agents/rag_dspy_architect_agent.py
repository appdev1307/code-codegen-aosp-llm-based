        ...
        if isinstance(content, dict):
            ...
            if content.get("header"):
                written.append(self._write(
                    ...
                    content["header"]))
            if content.get("impl"):
                # Impl goes to VehicleService{Domain}.cpp (matches Android.bp srcs in VssGlueAgent)
                written.append(self._write(
                    f"{self._CPP_DIR}/VehicleService{domain_cap}.cpp",
                    content["impl"]))
            # main_service and android_bp are NOT generated by domain agents
            # — VssGlueAgent generates VssVehicleService.cpp and the unified Android.bp
        else:
