...
def _generate_vss_hardware_cpp(props: list[dict], domains: list[str] = None) -> str:
    """VssVehicleHardware aggregates all domain services (each extends IVehicleHardware).
    Domain VehicleHalService*.cpp files implement per-domain prop configs via LLM+RAG.
    ...
    domain_configs = "\n".join(
        f"""    {{
        VehicleHalService{d.capitalize()} svc_{d};
        auto svc_cfgs = svc_{d}.getAllPropertyConfigs();
...
VssVehicleHardware::VssVehicleHardware() {{
    ...
    {init_lines}
    LOG(INFO) << "[VSS] Loaded " << mPropIds.size() << " properties";
}}
...
class VssGlueAgent:
    ...
    artifacts = {
            ...
            "VssVehicleHardware.cpp": _generate_vss_hardware_cpp(props, active_domains),
